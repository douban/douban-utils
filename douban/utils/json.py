# pylint: disable=E1103,W0312,W0402,W0621,W0704,W0201,R0903,R0911,R0912,
import sys, string
import types
import datetime
...
class JsonWriter(object):
    ...
    def _write(self, obj):
        ...
        if ty is types.DictType or isinstance(obj, dict):
            ...
            obj = obj.replace('\xe2\x80\xa9','')
            obj = obj.replace('\xe2\x80\xa8','')
            ...
            self._write(obj.strftime("%s %s" %(DATE_FORMAT, TIME_FORMAT)))
        elif ty is datetime.date:
            self._write(obj.strftime("%s" %(DATE_FORMAT)))
        elif ty is datetime.time:
            self._write(obj.strftime("%s" %(TIME_FORMAT)))
        else:
            raise WriteException, "Cannot write in JSON: %s" % repr(obj)

def write(obj, escaped_forward_slash=True):
    return JsonWriter().write(obj, escaped_forward_slash)

def json_encode_for_app(obj):
    return JsonWriter().write(obj, False)
...
try:
    import yajl
    simple_write = write
    def encode(obj, escaped_forward_slash=True):
        try:
            s = yajl.dumps(obj)
            if escaped_forward_slash:
                s = s.replace('/', r'\/')
        except TypeError:
            print ">>>>> error while yajl.dumps"
            print obj
            print ">>>>> end"
            import traceback; traceback.print_stack(limit=3, file=sys.stdout)
            #s = simple_write(obj, escaped_forward_slash)
            raise
        return s

    WriteException = TypeError
    ReadException = ValueError
    read = decode = yajl.loads
except ImportError:
    from warnings import warn
    warn('no yajl module available, please install it')
